...
def main(args=None):
    ...
    for batch in range(0, len(test_id), batch_size):
        ...
        y_hat = minmax_scaler.inverse_transform(y_hat.reshape(_batch_size*length,long_dim)) # JM Transform back
    ...
    for i in range(d_long):
        ...
        temp_result["Y"+str(i+1)+"err"] = np.sqrt(temp_result["Y"+str(i+1)+"err"].item()) # JM RMSE
